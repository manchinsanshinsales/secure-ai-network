...
### 3. メール送受信の設定 (対話型セットアップ)

実際のメール送受信やローカル環境でのテストを行うには、プロジェクトのルートディレクトリで以下の対話型セットアップスクリプトを実行してください。

```bash
# 対話式セットアップスクリプトの実行
./setup_approval_flow.sh
```

このスクリプトを実行すると、以下の2つのモードから選択できます：

1. **本物のメールサーバーを使用する (1)**
   - 送信元ボットの Gmail アドレスとアプリパスワードを入力し、実際の SMTP / IMAP サーバーへの接続をテストします。成功すると自動的に `.env` ファイルが作成されます。

2. **ローカルのモックメールサーバーを使用する (2) - ポートフォリオ確認向け 🌟**
   - Gmail の設定や資格情報の入力なしに、ローカル環境でメール承認フローを即座に試すことができます。
   - バックグラウンドでモックの SMTP / IMAP サーバー (`app/mock_mail_server.py`) が起動し、自動的に `bot@local.test` から `makoto.insidesales@gmail.com` への承認依頼メールが送信され、2秒後に自動的に `APPROVE` の返信がモック送信されるため、一連の承認実行フローをローカル完結でテストできます。

手動で設定する場合は、`.env` をルートディレクトリに作成し、以下のように定義します：
